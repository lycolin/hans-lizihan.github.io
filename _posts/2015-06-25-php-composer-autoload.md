---
layout:     post
title:      深入学习composer autoload 
date:       2015-06-25 12:12
summary:    php 基础之 composer autoload
categories: php
---
...
然后 composer 在背后就会读取这个文件夹中所有的文件 然后再 `vendor/composer/autoload_classmap.php` 中怒将所有的 __class__  的 namespace + classname 生成成一个 key => value 的 php 数组
...
好吧 上面的例子其实有点 tricky 就是上面这个 autoload 实际上是根据 prs-4 来生成出来的。不过这不重要，了解底层重要点，我们可以看到所有的所谓的 autoloading 其实可以理解为生成了这么一个 `classmap`，这是 `composer dump-autoload -o` 做的事儿。不然的话`compoesr` 会吭哧吭哧地去动态读取 psr-4 和 prs-0 的内容。
...
`Compsoer\ClassLoader` 会优先查看 `autoload_classmap` 中所有生成的注册类。如果在`classmap` 中没有发现再 fallback 到 psr-4 然后 psr-0

所以当打了 `composer dump-autoload -o` 之后，composer 就会提前加载需要的类并提前返回。这样大大减少了 IO 和深层次的 loop。

* 参考
  [composer 文档](https://getcomposer.org/doc/04-schema.md)
